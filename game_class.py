...
class Player:
    def __init__(self, startx, starty, car_speed, img_car):
        self.x = startx
        self.y = starty
        self.direction = 0 # 0:時計周り, 1:反時計回り
        self.speed = car_speed # 車のスピード
        self.img_car = img_car #車画像
        self.arg = -90 #現在の角度を保存する変数(度)
        self.w = img_car.get_width() #車の幅
        self.h = img_car.get_height() #車の高さ
        self.car_range = 80 #車の描画範囲(四角形で指定するための一辺の長さ)
        ### ゴールに必要なもの ###
        self.t1 = time.time() #システムが動き出すときの時刻
        self.tg = 0           #ゴールの瞬間の時刻
        self.goaltime = 0     #システム起動からゴールまでの時間
        self.goal_num = 3 #あと何周したらゴールになるかの変数


    def move_car(self):
        key = pygame.key.get_pressed()
        
        arg_rad = self.arg * pi / 180 #argをラジアンの単位に変換        
        arg_rotate = 10 #1回の回転角度

        #UPキーが押されたら
        if key[pygame.K_UP] == 1:
            self.x -= self.speed * sin(arg_rad)
            self.y -= self.speed * cos(arg_rad)
        #DOWNキーが押されたら
        ...
        if key[pygame.K_RIGHT] == 1:
            self.arg -= arg_rotate
            self.w = img_rotate.get_width()
            self.h = img_rotate.get_height()
        #LEFTキーが押されたら
...
class Game:
    ...
    def __init__(self, w = 1250, h = 750):
        # self.net = Network()
        self.index = 0
        #イベント毎に処理を分けるための変数
        # 0 => スタート画面
        ...
        self.width = w
        self.height = h
        # Player (self, startx, starty, car_speed, img_car)
        self.player1 = Player(550, 60, 15, pygame.image.load("img/car_3.png"))
        # self.player2 = Player(100, 100)
        self.canvas =Canvas(self.width, self.height, "脈拍カーレース")
        
    def start_display(bg): # index = 1
...
class Canvas:
    ...
    def __init__(self, w, h, mass, name="None"):
        self.map_w = w
        self.map_h = h
        self.map_mass = mass
        self.map_size = (self.map_w * self.map_mass, self.map_h * self.map_mass)
        self.screen = pygame.display.set_mode(map_size)
        pygame.display.set_caption(name)
        self.map = np.zeros((MAP_H, MAP_W), dtype=np.int)  #0で初期化
        
    def make_map():
        #周りの壁を作成
        self.map[0:MAP_H:MAP_H-1,:] = 1
        self.map[:,0:MAP_W:MAP_W-1] = 1

        #コース作成
        self.map[3,3:MAP_W-3:1] = 1
        self.map[3:12:1,3] = 1
        self.map[11,3:MAP_W-3:1] = 1
        self.map[6,6:MAP_W-1:1] = 1
        self.map[7:9:1,6:MAP_W-3:6] = 1
        self.map[9:11:1,9:MAP_W-3:6] = 1
